...
def detect_tool(data: dict, arg_tool: str = None) -> str:
...
def normalize(data: dict, tool: str) -> dict | None:
    ...
    base = {
        ...
        "repo": git("git remote get-url origin").split("/")[-1].replace(".git", "") or "unknown",
        "branch": git("git rev-parse --abbrev-ref HEAD") or "unknown",
        "commit": git("git rev-parse --short HEAD") or "unknown",
        "student": git("git config user.email") or "unknown",
    }
...
def main():
    ...
    parser.add_argument("--tool", help="Specify the AI tool name")
